...
import cn.yunchuang.im.pulltorefresh.recyclerview.MRecyclerView;
...
public abstract class AsyncLoadingAdapter<T extends BaseHolder> extends RecyclerView.Adapter<T> {
    ...
    public boolean setItemLimit(int limit) {
        if (limit == Integer.MAX_VALUE) {
            QLogUtil.e("Item limit should be less than Integer.MAX_VALUE " + Integer.MAX_VALUE);
            return false;
        }
        mItemLimit = limit;
        return true;
     ...
     * @return 实际加载的项数
     */
    protected abstract void onLoadMore(int requestSize);
    ...
    private synchronized void loadMore() {
        ...
        Runnable loadRunnable = new Runnable() {
            ...
            public void run() {
                ...
                int checkCount = 0;
                while (!readyForLoadMore(increment) && checkCount++ < MAX_BLOCK_LOOP_CNT) {
                    QLogUtil.w("Block load more until ready!");
                    try {
    ...
    protected void onloadMoreComplete() {
        ((Activity) getContext()).runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                QLogUtil.e("mlr 线程结束 onloadMoreComplete");
                mLoading = false;
                //检查是否到底了
                traceToEnd();
            }
...
//        QLogUtil.e("qbaoman traceToEnd mToEndEnabled:" + mToEndEnabled + "  mCanScrollUp:" + mCanScrollUp + " hasMore():" + hasMore());
...
//            QLogUtil.e("qbaoman traceToEnd  firstVisibleItem:" + firstVisibleItem);
            // 到底视图可用时 往上滑动 数据需要重刷
...
//                });
    ...
    public final T onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewHolder == null) {
            QLogUtil.e("Found NULL view at " + viewType + "!", new RuntimeException());
        }
        return viewHolder;
    }
    ...
    public final void onBindViewHolder(final T holder, final int position) {
        final int itemCount = getCount();
        if ((itemCount - getHeaderCount() - getFixedCount() > getPreloadCount())
                && (position >= itemCount - 1 - getPreloadCount())
                && (itemCount < getItemLimit()) && hasMore()
                && mMoreEnabled) {
            // load more items
            QLogUtil.e("mlr onBindViewHolder 开始调用 loadMore:");
            loadMore();
        }
