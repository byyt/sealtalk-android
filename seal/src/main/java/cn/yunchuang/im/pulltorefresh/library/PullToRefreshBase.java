...
import cn.yunchuang.im.pulltorefresh.library.internal.*;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    public PullToRefreshBaseOnInterceptTouchListener getPullToRefreshBaseOnInterceptTouchListener() {
        return pullToRefreshBaseOnInterceptTouchListener;
    }

    public void setPullToRefreshBaseOnInterceptTouchListener(PullToRefreshBaseOnInterceptTouchListener pullToRefreshBaseOnInterceptTouchListener) {
        this.pullToRefreshBaseOnInterceptTouchListener = pullToRefreshBaseOnInterceptTouchListener;
    }

   private   PullToRefreshBaseOnInterceptTouchListener pullToRefreshBaseOnInterceptTouchListener;

    public PullToRefreshBaseOnInterceptTouchListener getPullToRefreshBaseOnInterceptTouchListener_2() {
        return pullToRefreshBaseOnInterceptTouchListener_2;
    }

    public void setPullToRefreshBaseOnInterceptTouchListener_2(PullToRefreshBaseOnInterceptTouchListener pullToRefreshBaseOnInterceptTouchListener_2) {
        this.pullToRefreshBaseOnInterceptTouchListener_2 = pullToRefreshBaseOnInterceptTouchListener_2;
    }

    private   PullToRefreshBaseOnInterceptTouchListener pullToRefreshBaseOnInterceptTouchListener_2;
    public interface PullToRefreshBaseOnInterceptTouchListener {
        boolean isInterceptTouch(MotionEvent ev);
    }
                ...
                if (isReadyForPull()) {
                    ...
                    if (absDiff > mTouchSlop
                        ...
                        if (mMode.showHeaderLoadingLayout() && diff >= 1f
                            ...
                            mLastMotionY = y;
                            mLastMotionX = x;
                            if (pullToRefreshBaseOnInterceptTouchListener != null){
                                pullToRefreshBaseOnInterceptTouchListener.isInterceptTouch(event);
                            }
                            ...
                            mLastMotionX = x;
                            mIsBeingDragged = true;
                            if (pullToRefreshBaseOnInterceptTouchListener_2 != null){
                                pullToRefreshBaseOnInterceptTouchListener_2.isInterceptTouch(event);
                            }
                            if (mMode == Mode.BOTH) {
    ...
    //回调滑动距离
    private void callbackY(int value) {
        if (mCurrentMode == Mode.PULL_FROM_START) {
            if (mState != State.REFRESHING && mState != State.RESET) {
                if (scrollYChangeListener != null) {
                    scrollYChangeListener.getScrollY(value, true);
                }
            }
        }
    }

    IScrollYChangeListener scrollYChangeListener;

    public void setScrollYChangeListener(IScrollYChangeListener scrollYChangeListener) {
        this.scrollYChangeListener = scrollYChangeListener;
    }

    public interface IScrollYChangeListener {
        void getScrollY(int valueY, boolean down);
    }
